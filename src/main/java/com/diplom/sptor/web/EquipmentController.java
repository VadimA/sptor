...
public class EquipmentController {
   ...
   public String remove(@PathVariable("documentId")
    ...
    @RequestMapping(value = "/techcard/all", method = RequestMethod.GET,
            produces = "application/json")
    public @ResponseBody List<TechnologicalCard> getAllTechCards(Model model) {
        return technologicalCardService.getAllCards();
    }

    @RequestMapping(value = "/techcard/{repairId}", method = RequestMethod.POST,
            produces = "application/json")
    public void updateStatusRepair(@PathVariable("repairId") int repairId,
        ...
        if(status==5){
            technologicalCard.setEnd_date(newDate);
        }
        else {
            technologicalCard.setStart_date(newDate);
        }
        technologicalCardService.updateCard(technologicalCard);

        changeStatusOfEquipment(technologicalCard.getEquipment(),statusNew, technologicalCard.getTypeOfMaintenance());
    }
    ...
    public void confirmRepair(@PathVariable("repairId") int repairId,
        ...
        if (description.length()!=0) {
            technologicalCard.addDescription(description);
        }
        technologicalCardService.updateCard(technologicalCard);
    }
    ...
    @RequestMapping(value = "/stock", method = RequestMethod.GET,
            produces = "application/json")
    public String getPageStock(Model model) {
        model.addAttribute("listTechCard", technologicalCardService.getAllCards());
        model.addAttribute("tech_status_one", technologicalCardService.getTechnologicalCardByStatus(statusService.getStatusById(1)).size());
        model.addAttribute("tech_status_two", technologicalCardService.getTechnologicalCardByStatus(statusService.getStatusById(2)).size() + technologicalCardService.getTechnologicalCardByStatus(statusService.getStatusById(3)).size());
        model.addAttribute("tech_status_three", technologicalCardService.getTechnologicalCardByStatus(statusService.getStatusById(4)).size());
